...
export default function CoCreationsPage() {
  // Co-creations from the original site
  const coCreations = [
    "shabah-el-rih",
    "snakes-and-ladders",
    "msafreen",
  ...
  const coCreationProjects = projectPages.filter((p) =>
    coCreations.includes(p.slug)
  );
  ...
  return (
    <div className="min-h-screen px-[6vw] py-20 md:px-[4vw]">
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-12">
          <h1 className="text-4xl font-medium text-white">Co-creations</h1>
        </div>
        <div className="grid grid-cols-1 gap-[11px] md:grid-cols-2 lg:grid-cols-3">
          {coCreationProjects.map((project) => (
            <Link
              key={project.slug}
              href={`/${project.slug}`}
